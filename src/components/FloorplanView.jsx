...
export default function FloorplanView() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full overflow-hidden bg-slate-50 font-['Jost']">
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {viewMode === "map" && (
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-[1000] flex flex-col items-center">
              <div
                className={`${UI_TRANSITIONS} mb-3 flex flex-col gap-1 bg-white p-1.5 rounded-2xl shadow-xl border border-slate-200 min-w-[160px] ${isFloorMenuOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}
              >
                {floors.map((floor) => (
                  <button
                    key={floor.id}
              ...
              <button
                ...
                className="flex items-center gap-3 bg-white text-[#102a43] px-6 py-3 rounded-xl shadow-xl border border-slate-200 hover:bg-slate-50"
              >
      ...
      {/* FIXED: Triggered by centralized state */}
      <VirtualTourEmbed
        isOpen={!!activeTour}
        url={activeTour?.url}
